...
//将fd 的EPOLLIN祖册到 epollfd只是的epoll内核事件表中，并选择是否启用et模式
...
void lt(epoll_event* events, int number, int epollfd, int listenfd){
    ...
    for(int i = 0; i < number; i++){
        ...
        if(sockfd == listenfd){
            ...
            addfd(epollfd, connfd, false); //禁用et
        }
...
void et(epoll_event* events, int number, int epollfd, int listenfd){
    ...
    for(int i = 0; i < number; i++){
        ...
        if(sockfd == listenfd){
            ...
            addfd(epollfd, connfd, true); //启用fd
        }
        else if(events[i].events & EPOLLIN){
            printf("event trriger once\n");
            //et模式，这段代码不会被重复触发，所以要循环把socket中的数据都读出来
            while(1){
                memset(buf, '\0', BUFFER_SIZE);
                int ret = recv(sockfd, buf, BUFFER_SIZE-1, 0);
                if(ret < 0){
...
int main(int argc, char* argv[]){
    ...
    while(1){
        //epoll_wait将所有就绪的内核时间表复制到第二个参数的events中
